...
SearchList::SearchList(QWidget* parent):QListWidget(parent),find_w(nullptr)
{
	Q_UNUSED(parent)
...
void SearchList::on_the_item_clicked(QListWidgetItem* item)
{
	...
	else
	{
		...
		if (base&&base->get_item_type()==ListItemType::ADD_USER_ITEM)
		{
			...
			//auto search_edit = dynamic_cast<SerachEdit*>(this->search_edit);
			//auto uid_str = search_edit->text();
			//QJsonObject json_object;
			//json_object["uid"] = uid_str;
			//QJsonDocument doc(json_object);
			//TcpMgr::getInstance()->send_data(CMD_SERACH_USER, doc.toJson());
			if (!this->find_w)
			{
...
void SearchList::slot_user_seracher(std::shared_ptr<SearchInfo> user_info)
{
