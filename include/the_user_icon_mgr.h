...
private:
	...
	signals:
		...
		void sig_the_update_the_user_icon(const QString& user_id,const QString &email, const QPixmap& user_icon);
		...
		void sig_applications_user_icon_loading_done(const QList<std::pair<QPixmap, QString>>& icon_list);//异步获取好友申请列表头像完成后发出该信号
public:
	...
	void get_applications_user_icon_async(const QList<QString> user_list);//异步获取好友申请列表头像
